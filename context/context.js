import { AppCore, ZipAction, watch, signals } from "@benev/slate";
...
export class OmniContext extends Context {
    ...
    #save_to_storage(state) {
        if (collaboration.client || collaboration.isJoining) {
            return;
        } // do not save for client in collaboration
        if (state.projectId) {
    ...
    #recreate_project_from_localstorage_state(state, media) {
    ...
    constructor(options) {
        ...
        this.#core = new AppCore({
            ...
            history_limit: 64,
            actions_blueprint: ZipAction.blueprint()(historical_actions)
        });
