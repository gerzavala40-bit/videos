...
export class Compositor {
    ...
    timecode = 0;
    timebase = 25;
    currently_played_effects = new Map();
    ...
    constructor(actions) {
        ...
        this.#on_playing();
        reactor.reaction(() => this.#is_playing.value, (is_playing) => {
            if (is_playing) {
    ...
    canvasElementDrag = {
        onDragStart: (e, sprite, transformer) => {
    ...
    compose_effects(effects, timecode, exporting) {
        if (!this.#recreated) {
            return;
        }
    ...
    get_effect_current_time_relative_to_timecode(effect, timecode) {
        const current_time = timecode - effect.start_at_position + effect.start;
        return current_time / 1000;
    }
    get_effects_relative_to_timecode(effects, timecode) {
    ...
    #update_currently_played_effects(effects, timecode, exporting) {
    ...
    async seek(timecode, redraw) {
        ...
        for (const effect of this.currently_played_effects.values()) {
            ...
            if (effect.kind === "video") {
                ...
                const element = video?.texture.baseTexture.resource.source;
                if (!redraw && element?.paused && this.#is_playing.value) {
                    await element.play();
                }
    ...
    #add_effects_to_canvas(effects) {
        for (const effect of effects) {
            if (effect.kind === "image") {
                ...
                const element = this.managers.videoManager.get(effect.id)?.sprite?.texture.baseTexture.resource.source;
                if (element) {
                    element.currentTime = effect.start / 1000;
                }
            }
    ...
    #remove_effects_from_canvas(effects, exporting) {
        for (const effect of effects) {
            if (effect.kind === "image") {
    ...
    #on_selected_canvas_object() {
        this.app.stage.on("pointerdown", (e) => {
            //@ts-ignore
            const selected_effect = e.target ? e.target.effect : undefined;
        ...
        this.app.stage.on("pointerup", (e) => {
            //@ts-ignore
            const selected_effect = e.target ? e.target.effect : null;
            if (selected_effect) {
    ...
    update_canvas_objects(state) {
        this.app.stage.children.forEach(object => {
            if (!(object instanceof PIXI.Rectangle)) {
                //@ts-ignore
                const object_effect = object.effect;
                const effect = state.effects.find(effect => effect.id === object_effect?.id);
                if (effect) {
    ...
    setOrDiscardActiveObjectOnCanvas(selectedEffect, state) {
        if (!selectedEffect) {
            if (this.selectedElement) {
                this.app.stage.removeChild(this.selectedElement?.transformer);
            }
