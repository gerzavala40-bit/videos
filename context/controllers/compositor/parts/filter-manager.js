...
export class FiltersManager {
    ...
    selectedFilterForEffect(effect, type) {
        if (!effect)
            return;
    ...
    addFilterToEffect(effect, type, recreate) {
        const object = this.#getObject(effect);
        if (!object) {
            return;
        }
        ...
        if (object.filters instanceof Array === false)
            object.filters = [];
        const alreadyHasThisFilter = object.filters.find((f) => f.name === type);
    ...
    removeFilterFromEffect(effect, type, recreate) {
        const object = this.#getObject(effect);
        if (!recreate) {
            this.actions.remove_filter(effect, type);
        }
        if (object.filters instanceof Array)
            object.filters = object.filters.filter((f) => f.name !== type);
        this.onChange.publish(true);
        this.compositor.app.render();
    }
    updateEffectFilter(effect, filterName, propertyPath, value) {
        const object = this.#getObject(effect);
        if (Array.isArray(object.filters)) {
            const filter = object.filters.find((f) => f.name === filterName);
            if (filter) {
    ...
    async createFilterPreviews(onCreatedPreview) {
