...
export class ImageManager extends Map {
    compositor;
    actions;
    ...
    async create_and_add_image_effect(image, state) {
    ...
    async add_image_effect(effect, file, recreate) {
    ...
    remove_image_from_canvas(effect) {
        const image = this.get(effect.id);
        if (image) {
