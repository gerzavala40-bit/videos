...
export class VideoManager extends Map {
    ...
    create_and_add_video_effect(video, state) {
    ...
    add_video_effect(effect, file, recreate) {
    ...
    remove_video_from_canvas(effect) {
        const video = this.get(effect.id);
        if (video) {
    ...
    draw_decoded_frame(effect, frame) {
        const video = this.get(effect.id)?.sprite;
        if (video) {
