...
export class TransitionManager {
    ...
    selectTransition(transition, recreate) {
        ...
        if (!alreadyAdded && !recreate) {
            this.actions.set_effect_start_position(transition.incoming, transition.incoming.start_at_position - halfDuration);
            this.actions.set_effect_start(transition.incoming, transition.incoming.start + halfDuration);
            this.actions.set_effect_end(transition.outgoing, transition.outgoing.end - halfDuration);
        }
        ...
        return {
            apply: async (state) => {
                ...
                const alreadyAdded = this.getTransition(transition.id);
                if (!alreadyAdded || recreate) {
                    this.#selectTransition({ ...transition, incoming, outgoing }, state, recreate);
    ...
    async updateTransition(state, propsToUpdate) {
        if (this.selected && propsToUpdate) {
            const newFullDuration = normalizeTransitionDuration(propsToUpdate.duration, 1000 / state.timebase);
            ...
            if (transition) {
                ...
                this.actions.set_effect_start_position(incoming, incoming.start_at_position + delta);
                this.actions.set_effect_start(incoming, incoming.start - delta);
                this.actions.set_effect_end(outgoing, outgoing.end + delta);
            }
        }
        if (this.selected) {
    ...
    async #selectTransition(transition, state, recreate) {
        if (recreate) {
            this.removeTransition(transition.id, recreate);
        }
        ...
        if (incoming && outgoing) {
            ...
            const filter = new PIXI.Filter(vertexShader, 
            //@ts-ignore
    ...
    #onReverse(func, onlyAfterComplete, tick) {
        let previousTime = 0;
    ...
    getTransitionDurationPerEffect(transition, effect) {
        let incoming = 0;
        let outgoing = 0;
        if (transition) {
    ...
    removeTransitionFromNoLongerTouchingEffects(state) {
        const touchingClips = this.findTouchingClips(state.effects);
        const stillTouchingIds = new Set();
