...
export class FFmpegHelper {
    ...
    async write_composed_data(binary, container_name) {
        await this.ffmpeg.writeFile(`${container_name}`, binary);
    }
    async merge_audio_with_video_and_mux(effects, video_container_name, output_file_name, media, timebase) {
        ...
        */
        const audio_from_video_effects = effects.filter(effect => effect.kind === "video" && !isEffectMuted(effect));
        // those below are new audio effects to merge
        const added_audio_effects = effects.filter(effect => effect.kind === "audio" && !isEffectMuted(effect));
        ...
        const filtered_audios = all_audio_effects.filter((element) => !noAudioVideos.includes(element.id));
    ...
    async get_muxed_file(name) {
        return await this.ffmpeg.readFile(name);
    }
    async get_frames_count(file) {
