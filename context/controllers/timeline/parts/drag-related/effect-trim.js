...
export class effectTrimHandler {
    ...
    #normalize_to_timebase_and_set(state) {
        const effect = state.effects.find(({ id }) => id === this.effect?.id);
    ...
    #get_pointer_position_relative_to_effect_right_or_left_side(clientX, state) {
        return (clientX - this.initial_x) * Math.pow(2, -state.zoom);
    }
    trim_start = (e, effect, side) => {
        this.dropped = false;
...
//# sourceMappingURL=effect-trim.js.map