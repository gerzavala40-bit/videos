...
export function makeReadChunk(file) {
...
export class Media extends Map {
    ...
    async getVideoFileMetadata(file) {
    ...
    async syncFile(file, hash, proxy, isHost) {
        const alreadyAdded = this.get(hash);
        if (alreadyAdded && proxy) {
            return;
        }
        if (file.type.startsWith("image")) {
            const media = { file, hash, kind: "image" };
            this.set(hash, media);
            if (isHost) {
            ...
            const media = { file, hash, kind: "video", frames, duration, fps, proxy: proxy ?? false };
            this.set(hash, media);
            if (isHost) {
                this.import_file(file, hash, proxy);
            }
            ...
            const media = { file, hash, kind: "audio" };
            this.set(hash, media);
            if (isHost) {
                this.import_file(file, hash, proxy);
            }
    ...
    async import_file(input, proxyHash, isProxy) {
        ...
        if (imported_file) {
            ...
            if (isProxy === false && proxyHash) {
                await this.delete_file(proxyHash);
            }
            ...
            check_if_duplicate.onsuccess = () => {
                ...
                if (not_duplicate) {
                    if (imported_file.type.startsWith("image")) {
                        const media = { file: imported_file, hash, kind: "image" };
                        ...
                        const media = { file: imported_file, hash, kind: "video", frames, duration, fps, proxy: isProxy ?? false };
                        files_store.add(media);
                        this.set(hash, media);
                        this.on_media_change.publish({ files: [media], action: "added" });
                    }
                    else if (imported_file.type.startsWith("audio")) {
                        const media = { file: imported_file, hash, kind: "audio" };
                        files_store.add(media);
                        this.set(hash, media);
                        this.on_media_change.publish({ files: [media], action: "added" });
                    }
    ...
    create_image_elements(files) {
        const images = files.map(({ file, hash }) => {
    ...
    create_audio_elements(files) {
        const audios = files.map(({ file, hash }) => {
    ...
    async create_video_elements(files) {
        const videos = [];
