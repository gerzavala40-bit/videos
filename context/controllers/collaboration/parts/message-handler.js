...
function handleAction(actionType, payload, host, broadcastAction, connection) {
    ...
    const specializedHandler = actionHandlers[actionType];
...
export class MessageHandler {
    collaboration;
    fileHandler;
    constructor(collaboration, fileHandler) {
        this.collaboration = collaboration;
        this.fileHandler = fileHandler;
    }
    async handleMessage(connection, event) {
        // 1) First, delegate file-related messages to onFileChunk
        this.fileHandler.onFileChunk(connection, event, 64, async (hash, file, proxy) => {
        ...
        if (typeof event.data === "string") {
            ...
            switch (parsed.type) {
                case "action":
                    handleAction(parsed.actionType, parsed.payload, !!this.collaboration.host, this.broadcastAction.bind(this), connection);
    ...
    async #waitForContextChange(parsedProjectId) {
        return new Promise(resolve => {
            const interval = setInterval(() => {
                try {
                    if (omnislate?.context?.state?.projectId
                        === parsedProjectId) {
    ...
    async #handleInitMessage(parsed, connection) {
        ...
        if (this.collaboration.host) {
            this.collaboration.connectedClients.forEach(client => {
                if (!connection || client !== connection) {
                    client.cable.reliable.send(JSON.stringify({ actionType, payload, type: "action" }));
                }
            });
        }
        if (this.collaboration.client) {
            this.collaboration.client.connection.cable.reliable.send(JSON.stringify({ actionType, payload, type: "action" }));
        }
    }
}
const actionHandlers = {
    ...
    add_filter(payload) {
        ...
        const effect = omnislate.context.state.effects.find(e => e.id === filter.targetEffectId);
        if (effect) {
