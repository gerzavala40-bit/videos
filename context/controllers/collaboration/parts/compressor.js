...
export class Compressor {
    collaboration;
    #decoderWorkers = new Map();
    #encoderWorkers = new Map();
    #alreadyCompressingVideos = new Map();
    #taskQueue = [];
    #maxWorkers = 8;
    constructor(collaboration) {
        this.collaboration = collaboration;
    }
    compressVideo(file, props) {
        const task = async () => {
            ...
            demuxer(file, encoder.worker, (config) => decoder.worker.postMessage({ action: "configure", config }), (chunk) => {
    ...
    async compressAllVideos(state) {
        for (const effect of state.effects) {
            if (effect.kind === "video") {
                const media = omnislate.context.controllers.media.get(effect.file_hash);
