...
export class FileHandler {
    ...
    #fileMetadata = {};
    receivedFiles = [];
    #syncedHashes = new Set(); // Files that are already synced
    #pendingSync = new Set(); // Files currently being sent
    binary_accumulators = {};
    levels = [
    ...
    sendChunk(compressedChunk, hash, dataChannel) {
        this.#sendChunkWithHash(dataChannel, compressedChunk, hash);
    }
    sendFile(file, hash, dataChannel, total) {
        ...
        const readNextChunk = () => {
            ...
            reader.onload = () => {
                ...
                if (offset < file.size) {
                    ...
                    dataChannel.send(JSON.stringify({ done: true, hash, filename: file.name, fileType: file.type, proxy: false }));
    ...
    async onFileChunk(connection, event, hashLength, onComplete, onProgress) {
    ...
    #sendChunkWithHash(dataChannel, chunk, hash) {
    ...
    #receiveChunkWithHash(event, hashLength) {
    ...
    sendFileMetadata(dataChannel, hash, file, proxy, total) {
        const metadata = { hash, size: file.size, name: file.name, type: file.type, total, proxy };
        dataChannel.send(JSON.stringify(metadata));
    }
    async getMissingFiles(state) {
        const missing = [];
    ...
    handleMissingFiles(missing, connection) {
    ...
    broadcastMedia(media, connection, proxy) {
