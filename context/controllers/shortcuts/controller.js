...
export class Shortcuts {
    context;
    actions;
    #shortcutsByAction = new Map(); // Lookup by action type
    #shortcutsByKey = new Map(); // Lookup by shortcut key
    ...
    register(shortcut, type, actionType, action) {
    ...
    updateShortcut(type, newShortcut, override) {
    ...
    #saveShortcuts() {
        const serializedShortcuts = JSON.stringify(Array.from(this.#shortcutsByAction.values()).map(({ type, actionType, shortcut }) => ({
            shortcut,
            type,
            actionType,
        })));
    ...
    #getDefaultActionForDescription(description) {
        switch (description) {
            case "Copy":
            ...
            case "Previous frame":
                return (state) => {
                    ...
                    this.controllers.compositor.seek(state.timecode - 1000 / state.timebase, true).then(() => this.controllers.compositor.compose_effects(state.effects, state.timecode));
                };
            case "Next frame":
                return (state) => {
                    this.controllers.compositor.set_video_playing(false);
