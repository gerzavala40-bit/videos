...
export const OmniText = shadow_component(use => {
    ...
    const selectedText = use.context.state.effects.find(e => e.id === manager.selectedText?.id);
    const [fonts, setFonts] = use.state([]);
    const [fontsDenied, setFontsDenied] = use.state(null); // when user granted permission or not
    use.once(async () => {
        try {
				...
				<div class=colors-cnt>
					...
					<div>
						${(selectedText?.fill ?? manager.textDefaultStyles.fill).map((fill, i) => html `
							<div class=flex>
								<sl-input @sl-change=${(e) => manager.set_fill(e, i)} size="small" type="color" id="color" name="color" .value=${fill}></sl-input>
				...
				<div class=stops-cnt>
					...
					<div>
						${(selectedText?.fillGradientStops ?? manager.textDefaultStyles.fillGradientStops).map((stop, i) => html `
							<div class=flex>
								<sl-input
									@sl-change=${(e) => manager.set_fill_gradient_stop(e, i)}
    ...
    const textEffects = use.context.state.effects.filter(e => e.kind === "text");
