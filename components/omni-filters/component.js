...
import { FilterSchemas } from "../../context/controllers/compositor/parts/filter-manager.js";
export const OmniFilters = shadow_component(use => {
    ...
    const [filterPreviews, setFilterPreviews, getFilterPreviews] = use.state([]);
    const selectedImageOrVideoEffect = use.context.state.selected_effect?.kind === "video" || use.context.state.selected_effect?.kind === "image"
        ? use.context.state.effects.find(effect => effect.id === use.context.state.selected_effect.id)
        : null;
    use.mount(() => {
    ...
    const imageAndVideoEffects = () => use.context.state.effects.filter(effect => effect.kind === "image" || effect.kind === "video");
    ...
    const renderDropdownInfo = () => {
        return Tooltip(circleInfoSvg, html `<p>Select video or image either from dropdown menu here, timeline or scene</p>`);
    };
    const renderControl = (filterType, propertyPath, config) => {
        ...
        switch (config.type) {
            case "number":
                return html `
					<sl-range
						label=${label}
						@sl-change=${(e) => filtersManager.updateEffectFilter(selectedImageOrVideoEffect, filterType, propertyPath, +e.target.value)}
						type="range"
						min="${config.min}"
						max="${config.max}"
            ...
            case "color":
                return html `
					<label>
						${label}:
						<input
							@change=${(e) => filtersManager.updateEffectFilter(selectedImageOrVideoEffect, filterType, propertyPath, e.target.value)}
							type="color"
							value="${config.default}"
						>
					</label>
            ...
            case "boolean":
                return html `
					<label>
						${label}:
						<input
							@change=${(e) => filtersManager.updateEffectFilter(selectedImageOrVideoEffect, filterType, propertyPath, e.target.checked ? 1 : 0)}
							type="checkbox"
            ...
            case "choice":
                ...
                return html `
					<sl-select
						...
						@sl-change=${(e) => filtersManager.updateEffectFilter(selectedImageOrVideoEffect, filterType, propertyPath, +e.target.value)}
					>
						${normalizedOptions.map(({ value, label }) => html `
            ...
            case "object":
                ...
                return html `
					<fieldset>
						<legend>${label}</legend>
						${Object.entries(config.properties).map(([subProp, subConfig]) => renderControl(filterType, [...propertyPath, subProp], subConfig))}
					</fieldset>
				`;
            case "array":
                return html `
    ...
    const renderFilterOptions = (type) => {
        const { ...params } = FilterSchemas[type];
        return html `
			${Object.entries(params).map(([propertyName, config]) => renderControl(type, [propertyName], config))}
		`;
    };
    const renderFilters = () => {
