...
export const OmniMedia = shadow_component((use) => {
    ...
    const [media, setMedia, getMedia] = use.state([]);
    const [placeholders, setPlaceholders, getPlaceholders] = use.state([]);
    ...
    use.mount(() => {
        media_controller.getImportedFiles().then(async (media) => {
            ...
            const video_files = media.filter(({ kind }) => kind === "video");
            const image_files = media.filter(({ kind }) => kind === "image");
            const audio_files = media.filter(({ kind }) => kind === "audio");
        ...
        const unsub = media_controller.on_media_change(async (change) => {
            if (change.action === "added") {
                const video_files = change.files.filter(({ kind }) => kind === "video");
                const image_files = change.files.filter(({ kind }) => kind === "image");
                const audio_files = change.files.filter(({ kind }) => kind === "audio");
                const video_elements = await media_controller.create_video_elements(video_files);
                const image_elements = media_controller.create_image_elements(image_files);
                const audio_elements = media_controller.create_audio_elements(audio_files);
                setMedia([...getMedia(), ...video_elements, ...image_elements, ...audio_elements]);
    ...
    const handleDrop = (e) => {
        ...
        if (e.dataTransfer?.files) {
            const fileInput = use.shadow.querySelector("#import");
            fileInput.files = e.dataTransfer.files;
            const event = new Event("change", { bubbles: true });
            fileInput.dispatchEvent(event);
        }
    };
    const render_video_element = (video) => {
        return html `
    ...
    const render_image_element = (image) => {
        return html `
    ...
    const render_audio_element = (audio) => {
        return html `
		...
		>
			<div class="header">
				<form>
					...
					<input type="file" accept="image/*, video/*, .mp3" id="import" class="hide" multiple @change=${(e) => media_controller.import_file(e.target)}>
				</form>
			</div>
			
			<div class="media-grid">
				${placeholders.map((_) => html `
