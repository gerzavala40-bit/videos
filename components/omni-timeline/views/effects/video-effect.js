import { html, css, watch } from "@benev/slate";
...
export const VideoEffect = shadow_view(use => (effect, timeline) => {
    ...
    const [visibleFilmstripFrames, setVisibleFilmstripFrames, getVisibleFilmstripFrames] = use.state([]);
    const { recalculate_filmstrip_frames, filmstrip } = use.once(() => {
        ...
        const recalculate_filmstrip_frames = async (force_recalculate) => {
            const get_effect = use.context.state.effects.find(e => e.id === effect.id);
...
//# sourceMappingURL=video-effect.js.map