...
export const Effect = shadow_view(use => (timeline, any_effect, content, style, inline_css) => {
    ...
    const [[x, y], setCords] = use.state([null, null]);
    ...
    const [previewPosition, setPreviewPosition] = use.state({
        start: null,
    ...
    const drag_events = {
        ...
        start(event) {
            ...
            if (bounds && !handler.grabbed) {
                ...
                const at = { coordinates: [x >= 0 ? x : 0, y >= 0 ? y : 0], indicator: null };
    ...
    const render_trim_handle = (side) => {
        return html `
			<span
				@pointerup=${(e) => handler.trim_drop(e, use.context.state)}
				@pointercancel=${(e) => handler.trim_end(e, use.context.state)}
				@pointerdown=${(e) => handler.trim_start(e, effect, side)}
