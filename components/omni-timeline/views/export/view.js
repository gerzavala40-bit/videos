...
export const Export = shadow_view(use => () => {
    ...
    const [logs, setLogs, getLogs] = use.state([]);
    use.mount(() => {
...
export const ExportInProgressOverlay = shadow_view((use) => () => {
    ...
    const formattedProgress = state.export_status === "complete" || state.export_status === "flushing"
        ? 100
...
export const ExportConfirmModal = shadow_view(use => (showModal, setShowModal) => {
    use.watch(() => use.context.state);
    use.styles([confirmModalStyles]);
    const mediaController = use.context.controllers.media;
    const [_, setFilesProgess, getFileProgress] = use.state(new Map());
    ...
    const getProxyPreview = (hash) => {
        return use.context.state.effects.find(e => e.kind === "video" && e.file_hash === hash).thumbnail;
    };
    const getFileMetadata = (hash) => {
