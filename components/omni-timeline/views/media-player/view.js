...
export const MediaPlayer = shadow_view(use => () => {
    ...
    use.mount(() => {
        const unsub_onplayhead1 = playhead.onPlayheadMove(async (x) => {
            if (use.context.state.is_playing) {
                compositor.set_video_playing(false);
            }
            compositor.compose_effects(use.context.state.effects, use.context.state.timecode);
            compositor.seek(use.context.state.timecode, true).then(() => compositor.compose_effects(use.context.state.effects, use.context.state.timecode));
        });
        const dispose1 = watch.track(() => use.context.state, async (timeline) => {
        ...
        const dispose2 = watch.track(() => use.context.state.timecode, (timecode) => {
