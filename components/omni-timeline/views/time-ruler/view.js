...
export const TimeRuler = shadow_view(use => (timeline) => {
    use.styles(styles);
    const [timeCodes, setTimeCodes] = use.state([]);
    const [_, setPrevTimecode, getPrevTimecode] = use.state(null);
    const [_p, setPrev, getPrev] = use.state(null);
    ...
    function convert_ms_to_timecode(milliseconds) {
    ...
    function round_to_two_decimal_places(zoom) {
        return Math.round(zoom * 100) / 100;
    }
    function round_timecode_to(timecode, zoom, ms) {
    ...
    function generate_time_codes(zoom) {
        const time_codes = [];
    ...
    const translate_to_timecode_and_set = (x) => {
    ...
    return html `
		<div
			...
			@pointermove=${(e) => setIndicatorX(e.clientX - timeline.getBoundingClientRect().left + timeline.scrollLeft)}
			@pointerdown=${(e) => setIndicatorX(e.clientX - timeline.getBoundingClientRect().left + timeline.scrollLeft)}
			@pointerup=${(e) => translate_to_timecode_and_set(e.clientX - timeline.getBoundingClientRect().left + timeline.scrollLeft)}
			class=time-ruler
		>
