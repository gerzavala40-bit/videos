...
export const OmniTimeline = shadow_component(use => {
    ...
    use.mount(() => {
        const layout = document.querySelector("construct-editor")?.shadowRoot?.querySelector(".layout");
        if (layout) {
    ...
    function augmented_dragover(event) {
    ...
    const timeline = use.defer(() => use.shadow.querySelector(".timeline-relative")) ?? use.element;
    return StateHandler(Op.all(use.context.helpers.ffmpeg.is_loading.value, use.context.helpers.ffmpeg.is_loading.value), () => html `
		${Toolbar([timeline])}
		<div
			class="timeline"
